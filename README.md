...
IMPORTANT
=========
This build is used as an experiment for a jsdom server implementation: 
     http://github.com/a-lucas/angular-on-server
...
#### - IdleState
```!javascript
window/$window.addEventListener( 'IdleState', function() {
    console.log('The AngularApp is now in an idle state.');
});
```
     
#### - AngularContextException
```!javascript
window/$window.addEventListener('AngularContextException', function(err){
    console.log('Angular notifies the renderer some s### just happened', err); 
});
```
